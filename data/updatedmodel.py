...
#plot model to an image
#plot_model(model, to_file = NAME + '.png')
#fit model and send data to tensorboard
model.fit(train_x, train_y, validation_data = (test_x, test_y), epochs = numepochs)
        
#reshuffle datasets for validation
rng_state = np.random.get_state()
np.random.shuffle(b)
np.random.set_state(rng_state)
np.random.shuffle(g)

generalpred = model.predict(b)

#smooths predicted data
for i in range(len(generalpred)):
    for j in range(3):
        if generalpred[i][j] == max(generalpred[i]):
            generalpred[i][j] = 1
        else:
            generalpred[i][j] = 0
...
count = 0
for i in range(len(generalpred)):
    if np.array_equal(generalpred[i], g[i]):
        count += 1
total = len(generalpred)
